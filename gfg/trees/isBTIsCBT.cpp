...
bool  checkForCBT(Node*root,int index,int nodes){
    if(root==NULL) return true;
    
    if(index>= nodes) return false;
    
    bool left=checkForCBT(root->left,2*index+1,nodes);
    bool right=checkForCBT(root->right,2*index+2,nodes);
    return left&&right;
}
    bool isCompleteBT(Node* root){
       int nodes=totalNodes(root);
       return checkForCBT(root,0,nodes);
    }
};
